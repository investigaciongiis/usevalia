 ...
 * @author luis
 */
class ReabrirAuditoriaForm extends FormBase {

  private $auditorias;
  private $usuario;

  public function getFormId() {
    return 'reabrir-auditoria-form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $output = [];
    $this->auditorias = $controlador->getAuditoriasFullCerradasFrom($this->usuario);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if(!empty($form_state->getValue('table'))){
        $controlador->reabrirAuditoria($this->auditorias[$form_state->getValue('table')]);
        $messenger->addMessage(t('La auditoría "'). $this->auditorias[$form_state->getValue('table')]->__get('nombre') . t('" se ha vuelto a abrir correctamente.'));
        $form_state->setRedirect('usevalia.reabrir_auditoria');
    }else{
